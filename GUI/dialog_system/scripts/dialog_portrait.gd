...
func check_mouth_open( l : String ) -> void:
	if 'aeiouyAEIOUY1234567890'.contains( l ):
		open_mouth = true
		mouth_open_frames += 2
		audio_stream_player.pitch_scale = randf_range( audio_pitch_base - 0.05, audio_pitch_base + 0.05 )
	...
	if mouth_open_frames == 0:
		if open_mouth == true:
			open_mouth = false
			audio_stream_player.pitch_scale = randf_range( audio_pitch_base - 0.09, audio_pitch_base + 0.02 )
			audio_stream_player.play()
	pass
