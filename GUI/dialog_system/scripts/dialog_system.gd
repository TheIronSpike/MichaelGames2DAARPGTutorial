...
@onready var nameplate: Label = $DialogUI/Nameplate
...
#where we handle our key presses while the Dialog System is active.
...
func set_dialog_text( _d : DialogText ) -> void:
	...
	nameplate.text = _d.npc_info.npc_name
...
#Set the Dialog Choice UI based on parameters
func set_dialog_choice( _d : DialogChoice ) -> void:
	...
	for i in _d.dialog_branches.size():
		...
		_new_choice.pressed.connect( dialog_choice_selected.bind( _d.dialog_branches [ i ] ) )
...
func dialog_choice_selected( _d : DialogBranch ) -> void:
	choice_options.visible = false
